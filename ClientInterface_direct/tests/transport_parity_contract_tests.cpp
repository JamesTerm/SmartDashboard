...
namespace sd::direct
{
    ...
    TEST(TransportParityContractTests, DirectDashboardSurvivesStressKeepsRobotOwnedChooserSelection)
    {
        ...
        robot.PutString("Test/AutoChooser/.type", "String Chooser");
        robot.PutStringArray("Test/AutoChooser/options", {"Do Nothing", "Just Move Forward", "Just Rotate"});
        robot.PutString("Test/AutoChooser/default", "Do Nothing");
        robot.PutString("Test/AutoChooser/active", "Just Move Forward");
        robot.PutString("Test/AutoChooser/selected", "Just Move Forward");
        ASSERT_TRUE(robot.FlushNow());
        ...
        ASSERT_TRUE(WaitUntil(
            [&dashboard, &selected]()
            {
                return dashboard.TryGetString("Test/AutoChooser/selected", selected);
            },
            2s
        ));
        ...
        ASSERT_TRUE(WaitUntil(
            ...
            {
                return dashboardRestarted.TryGetString("Test/AutoChooser/selected", selected);
            },
            2s
        ));
        EXPECT_EQ(selected, "Just Move Forward");
