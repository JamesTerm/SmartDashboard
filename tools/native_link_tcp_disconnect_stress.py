"""
Native Link TCP disconnect/reconnect stress test.
...
The TCP carrier's auto-reconnect loop (RunLoop in native_link_tcp_client.cpp) retries
every 500ms after a drop.  That's faster than the SHM restart window, so the
Connected-state timeout is kept at 10s per cycle (same as the SHM script).

Usage:
    python tools/native_link_tcp_disconnect_stress.py [cycles] [pause_ms]
...
Ian: This script is the TCP peer of native_link_shm_disconnect_stress.py.
Keep the two in sync when adding new cycle assertions or helper functions.
"""
import ctypes
...
def configure_registry() -> None:
    settings = [
        ...
        ("ntClientName", "REG_SZ", "tcp-stress-dashboard"),
        (
            "pluginSettingsJson",
            "REG_SZ",
...
def main() -> int:
    ...
    # Authority must start first so the TCP listener is up before the dashboard dials.
    ...
    try:
        ...
        # Ian: TCP auto-reconnect means the dashboard may still be draining the
        # initial snapshot when the first cycle fires.  Waiting for the first
        # "connection_state=Connected" entry guarantees the snapshot handshake is
        # complete before we start disconnecting.  The same rationale applies as in
        # the SHM stress script (see alive-guard note in dashboard_transport.cpp).
        ...
        for cycle in range(1, cycles + 1):
            ...
            # --- Reconnect ---
            # Ian: For TCP, the "connect" command tells the carrier to re-enter its
            # RunLoop immediately rather than waiting for the 500ms auto-retry timer.
            # This keeps cycle timing comparable to the SHM script.
        ...
        print("native_link_tcp_disconnect_stress=ok")
        return 0
    finally:
        stop_processes()
